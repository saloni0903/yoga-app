...
export default function DashboardPage({ darkMode }) {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const statsPromise =await authFetch("/api/admin/stats");
        const pendingPromise =await authFetch(
          "/api/admin/instructors?status=pending&limit=5",
        );
        const chartPromise =await authFetch(
          "/api/admin/stats/attendance-over-time?period=30",
        );
        const activityPromise =await authFetch("/api/admin/activity-feed?limit=7");
        const topGroupsPromise =await authFetch(
          "/api/admin/stats/top-groups?limit=5",
        );

        const [statsRes, pendingRes, chartRes, activityRes, topGroupsRes] =
          await Promise.all([
            statsPromise,
            pendingPromise,
            chartPromise,
        ...
        if (
          [statsRes, pendingRes, chartRes, activityRes, topGroupsRes].some(
            (res) => res.status === 401,
          )
        ) {
        ...
        if (!pendingRes.ok) {
          console.error(
            `Pending Instructors API failed with status ${pendingRes.status}`,
          );
        ...
        if (!activityRes.ok) {
          console.error(
            `Activity Feed API failed with status ${activityRes.status}`,
          );
          setActivityFeed([]);
        } else {
        ...
        if (!topGroupsRes.ok) {
          console.error(
            `Top Groups API failed with status ${topGroupsRes.status}`,
          );
          setTopGroups([]);
        } else {
  ...
  const renderActivityItem = (item) => {
    ...
    switch (item.type) {
      case "USER_REGISTERED":
        ...
        text = (
          <p>
            <strong>{item.details.name}</strong> just registered as a
            participant.
          </p>
        );
        break;
      case "SESSION_COMPLETED":
        ...
        text = (
          <p>
            <strong>{item.details.groupName}</strong> (w/{" "}
            {item.details.instructorName}) just completed a session.
          </p>
        );
        break;
      case "INSTRUCTOR_APPROVED":
        ...
        text = (
          <p>
            You approved <strong>{item.details.name}</strong> as an instructor.
          </p>
        );
        break;
      default:
    ...
    return (
      <div
        key={item.id}
        className="flex space-x-3 pb-3 border-b border-gray-200 dark:border-gray-700 last:pb-0 last:border-b-0"
      >
        <div
          className={`mt-1 p-1.5 rounded-full h-fit ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
        >
          {icon}
        </div>
  ...
  if (error) {
    return (
      <div
        className={`p-4 rounded-md ${darkMode ? "bg-red-900/30 text-red-400" : "bg-red-100 text-red-700"}`}
        role="alert"
      >
  ...
  return (
    <div>
      <div className="mb-8">
        <h3
          className={`text-3xl lg:text-4xl font-bold mb-2 ${darkMode ? "text-white" : "text-gray-800"}`}
        >
          Dashboard Overview
        </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        ...
        <div
          className={`p-6 rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
            darkMode
      ...
      <div
        className={`mt-8 rounded-2xl shadow-lg p-6 h-80 ${
          darkMode
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* --- WIDGET 1: PENDING INSTRUCTORS --- */}
        <div
          className={`lg:col-span-1 rounded-2xl shadow-lg p-6 transition-colors duration-300 ${
            darkMode
              ? "bg-gray-800 border border-gray-700"
              : "bg-white border border-gray-200"
          }`}
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto">
            {pendingInstructors.length > 0 ? (
              pendingInstructors.map((inst) => (
                <div
                  key={inst._id}
                  className="flex items-center space-x-3 p-2 rounded-lg transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50"
                >
                  <div
                    className={`p-2 rounded-full ${darkMode ? "bg-gray-700" : "bg-yellow-100"}`}
                  >
                    <UserPlus
              ...
              <div
                className={`text-center py-4 rounded-xl ${darkMode ? "bg-gray-700/50" : "bg-gray-50"}`}
              >
                <Award
                  className={`w-6 h-6 mx-auto mb-2 ${darkMode ? "text-green-400" : "text-green-600"}`}
                />
                <p
            ...
            {topGroups.length > 0 ? (
              topGroups.map((group, index) => (
                ...
                >
                  ...
                  <Link
                    to={`/groups/${group.id}`} // Assumes you'll have a /groups/:id route
                    className={`p-1 rounded-full ${darkMode ? "text-teal-400 hover:bg-gray-700" : "text-teal-600 hover:bg-gray-100"}`}
                  >
                    <ChevronRight className="w-4 h-4" />
                  </Link>
