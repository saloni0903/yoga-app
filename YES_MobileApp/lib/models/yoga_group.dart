...
class Schedule {
  ...
  factory Schedule.fromJson(Map<String, dynamic> j) {
    return Schedule(
      startTime: j['startTime']?.toString() ?? '00:00',
      //startTime: j['startTime'] ?? '00:00',
      endTime: j['endTime']?.toString() ?? '00:00',
...
class YogaGroup {
  ...
  factory YogaGroup.fromJson(Map<String, dynamic> j) {
    ...
    return YogaGroup(
      ...
      name: (j['group_name'] ?? j['groupName']).toString(),
      //groupType: j['groupType'] as String? ?? 'offline',
      groupType: j['groupType']?.toString()?? 'offline',
      location:
          (j['location']
      ...
      maxParticipants: int.tryParse(j['max_participants']?.toString() ?? '20') ?? 20,
      //pricePerSession: ((j['price_per_session'] ?? 0) as num).toDouble(),
      pricePerSession: double.tryParse(j['price_per_session']?.toString() ?? '0') ?? 0.0,
      ...
      equipmentNeeded: (j['equipment_needed'] as List? ?? [])
      .map((e) => e.toString())
      .toList(),
      instructorId: getInstructorId(j['instructor_id']),
      //memberCount: j['memberCount'] as int?,
      memberCount: int.tryParse(j['memberCount']?.toString() ?? '')?? 0,
