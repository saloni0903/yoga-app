...
class _InstructorDashboardState extends State<InstructorDashboard> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Instructor Dashboard', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
            Text(
  ...
  Widget _buildPendingApprovalScreen(BuildContext context, ApiService apiService, User currentUser) {
    return Scaffold(
      appBar: AppBar(
...
class _GroupListItem extends StatelessWidget {
  ...
  Future<void> _generateQrCode(BuildContext context, ApiService api, User user) async {
    ...
    try {
      final SessionQrCode qrCode = await api.qrGenerate(groupId: group.id, sessionDate: DateTime.now(), createdBy: user.id);
      if (!context.mounted) return;
      Navigator.pop(context);
      Navigator.push(context, MaterialPageRoute(builder: (_) => QrDisplayScreen(qrCode: qrCode, groupName: group.name)));
    } catch (e) {
      if (!context.mounted) return;
      Navigator.pop(context);
